...
export function loadConfig() {
  ...
  const c = normalizeKeys({ ...DEFAULT_CONFIG, ...raw });
  // One-time upgrade: older installs defaulted to `.mkv` only. Add `.mp4` unless
  // the user has already been migrated, so a deliberate later choice isn't undone.
  if (!c.mp4Migrated) {
    if (Array.isArray(raw.formats) && raw.formats.length === 1 && raw.formats[0] === '.mkv') {
      c.formats = ['.mkv', '.mp4'];
    }
    c.mp4Migrated = true;
    try { writeJson(CONFIG_FILE, c); } catch { /* read-only fs — apply in memory only */ }
  }
  return c;
}
